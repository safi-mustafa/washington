...
function fnIsReStageMaterialValid() {
    ...
    $('.transactions-checkbox:checked').each(function () {
        ...
        var location = $('#location-id-' + index).val(); // Get the selected location
        var quantity = parseInt($('input[name="Transactions[' + index + '].NewQuantity"]').val()); // Get the entered quantity

        // Check if location and quantity are set for the current checked item
        if (!location || isNaN(quantity) || quantity <= 0) {
...
function GetTransactions(lotNo) {
    ...
    if (lotNo.length > 0) {
        $('#ajax-loader').show();
        $.get("/Execute/GetItemsToRestage?lotNo=" + lotNo, function (data, status) {
...
function fnSaveRestageInventory() {
    ...
    var isValid = fnIsReStageMaterialValid();
    if (isValid) {
        let formData = getSelectedTransactionItems();
        $.ajax({
            url: "/Execute/ReStageItems",
            type: "POST",
            ...
            success: function (response) {
                if (response) {
                    SwalSuccessAlert("Successfully Re-staged items.");
                    window.location = "/Inventories?activeTab=execute";
                }
                else {
                    SwalErrorAlert("Error: Unable to Re-stage items. Please try again.");
                }
            },
            complete: function () {
            },
        ...
        SwalErrorAlert('Please make sure location and quantity are valid for all checked items, and quantity is greater than 0.');
    }
}
